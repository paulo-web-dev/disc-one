...
<div class="kpi-grid">
    <div class="kpi">
        <div class="top"><div class="ic" style="background: rgba(0,163,255,0.12); color: var(--brand-300);">📋</div></div>
    ...
    <div class="kpi">
        <div class="top"><div class="ic" style="background: rgba(110,139,255,0.12); color: var(--disc-c);">👥</div></div>
        <div class="label">Respondentes</div>
        <div class="value">{{ $respondents }}</div>
        <div class="sub">contas cadastradas</div>
    </div>
    <div class="kpi">
        <div class="top"><div class="ic" style="background: rgba(255,84,112,0.12); color: var(--disc-d);">R$</div></div>
    ...
    <div class="panel">
        ...
        @if ($totalDist > 0)
            ...
            <div class="legend" style="justify-content: center; gap: 18px; margin-top: 8px;">
                <span><span class="sw" style="background: #FF5470;"></span>D · {{ $distribution['D'] }}</span>
                <span><span class="sw" style="background: #FFB547;"></span>I · {{ $distribution['I'] }}</span>
                <span><span class="sw" style="background: #2BD9A1;"></span>S · {{ $distribution['S'] }}</span>
                <span><span class="sw" style="background: #6E8BFF;"></span>C · {{ $distribution['C'] }}</span>
            </div>
        @else
        ...
        @if ($totalTests > 0)
            <div style="padding: 16px 0;">
                ...
                <div style="display: flex; justify-content: space-between; font-size: 14px; color: var(--fg-2); margin-bottom: 8px;">
                    <span>Concluídos</span><span><b style="color:#fff;">{{ $completed }}</b> / {{ $totalTests }}</span>
                </div>
